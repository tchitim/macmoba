...
最後更新：2026-08-19 · 版本 **v1.85** · 測試 **739 項全過**
（**「no remote desktop in this window」——遠端桌面明明就在畫面上**。`focusedFramebufferView` 只看 `NSApp.keyWindow`,而**選單動作執行的當下 keyWindow 可能是 nil**（選單追蹤尚未完全結束）,多視窗時也會找錯窗。改成:焦點優先,接著依序找 keyWindow → mainWindow → 所有可見視窗。這同時解釋了 ⌥⌘V「never used」:鍵盤那條路的判斷式同樣以它為前提,找不到就整個跳過）
