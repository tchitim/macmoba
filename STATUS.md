...
最後更新：2026-09-12 · 版本 **v3.0** · 測試 **843 項**（3 項 keychain 測試在無互動的終端機下會失敗,見文末）
（**libghostty 成為所有人的預設引擎,SwiftTerm 留在設定裡**(3.0)。本機連續測了幾天、從 3.07 一路修到 3.18 才做這件事——期間冒出來的每一個 bug 都是**只有真的在用才會撞到**的那種:⌘V 沒經過選單驗證、⌘T 被 keybind 吃掉、滑鼠追蹤沒關掉、貼圖片變成貼路徑、以及複製被一個 weak 參考停用。當初列為「還沒跟上」的三項現在都補完了:⌘F 與 read-screen 讀的是**整個 scrollback** 而不只是 viewport(在 vendored 套件加了 `readAllText()`)、主題以 config text 送進去、Select All **本來就不缺**——缺的是套件走的那條參考。`AppTerminalView.selectAll` 經過 coordinator,而 coordinator **持有** surface(強參考);`TerminalViewState.surface` 是 weak 的鏡像,就是它變成 nil 之後在 3.15 把 Copy 整個停用掉。**SwiftTerm 不是棄用**:設定裡一鍵切回去,而且使用者自己的選擇會**永久**壓過建置預設——因為某個東西壞掉才切回去的人,不該被下一次更新又搬走。plist 的鍵現在兩個方向都能釘(`swiftterm` 也認),所以要 bisect 不必改原始碼;認不得的值退回**預設**而不是另一個引擎,免得建置腳本打錯字就靜悄悄出貨了沒人要的引擎。**誠實記下效益的大小**:進本機 shell 快 2.3 倍,但在真實 SSH 連線上**量不出差別**(p = 0.92)——網路先撐不住,兩個 parser 都還沒到極限。贏的是本機 shell、序列埠、以及任何會狂噴輸出的東西。)
（**網頁分頁可以對應解不到的名稱**(3.0)。內部名稱只存在於某台碰不到的 DNS,Mac 和跳板機都解不到,頁面在用到隧道之前就死了。Chrome 用 `--host-resolver-rules="MAP name 10.0.0.1"` 解決,而那個旗標**為什麼是對應而不是直接換位址**才是重點:名稱本身還得送出去,TLS 拿它比對憑證、多站主機靠 SNI 和 Host 標頭分辨。所以替換只發生在**一個地方**——SOCKS CONNECT 轉成 channel 的那一行——上游完全不知情。規則放在 web 連線上而不是它借道的跳板機:網路嚴格說是跳板機的屬性,但需要這個的人是剛看著頁面連不上的那個人,而他打開的是 web 連線。存的是**打進去的原文**不是解析後的表,所以解析器略過的那一行下次還在原地等著被修。也支援 `name = 位址:port`,IPv6 依 URL 的規矩(要加括號才切 port)。⚠️ **使用者實測到這裡為止**:對應確實生效了,但那台 ELB 回 404(`X-ROUTER-STATUS: MISS`)——用 `curl --resolve` 在跳板機上跑得到**一模一樣**的 404,所以是對面沒有這條路由,不是 MacMoba。)
（**本地建置預設 libghostty、發布版維持 SwiftTerm——用建置旗標而不是改原始碼**(2.99,使用者驗收:`engine = libghostty`)。需求是「本地的 dmg 都開啟 libghostty,git 的 release 才保留 SwiftTerm」。**沒有**在分支上把 `?? false` 改成 `?? true`——那會變成一個「要記得改回去的分支」,而要記得的事最後都會忘記、遲早混進正式發布。改成 `make-app.sh` 只在 `GHOSTTY_DEFAULT=1` 時往 Info.plist 寫 `MacMobaDefaultEngine`;**兩個分支的腳本完全一樣**,發布版要帶上那個預設得有人主動打環境變數。優先順序:使用者自己設的值 > 建置宣告 > SwiftTerm——使用者的選擇**兩個方向都要贏**,否則沒辦法從 libghostty 版切回去。順帶補上 `list-tabs` 的 `engine` 欄位:使用者會問「這版是用 libghostty 嗎」正是因為**看不出來**,而效果看不見的設定沒有人會信任;`mixed` 是可能的,因為引擎在 pane 建立時就決定了。⚠️ **端到端在這台驗不了**:System Events 中途開始列不出 MacMoba 的視窗、也叫不到前景,我一度以為是自己改壞了、還回到上一個 commit 重建確認,最後直接問 window server 才確定 **app 一直有一個正常的螢幕上視窗**——是自動化工具壞了不是程式。最後由使用者確認 `engine = libghostty`。）
（**右鍵誤觸會靜默上傳一張截圖到遠端——現在滑鼠貼上只貼文字**。使用者:「我沒有 copy paste,為什麼還有這些」。不是 ⌘V。`ClipboardTerminalView.rightMouseDown` 在 `mousePaste`(**預設開啟**,MobaXterm 的習慣)下**直接就是貼上**,中鍵同理(xterm 的習慣);而 `requestPaste` 的第一件事就是「SSH pane + 剪貼簿是圖片 → SFTP 上傳成檔案」,**不問、不確認**。所以只要剪貼簿裡還躺著稍早 ⇧⌘4 的截圖,在終端機裡右鍵一下(多數人以為那是開選單)就會在遠端寫一個檔案並把路徑打進提示字元。時間戳對得上,而且那 30 個累積檔案裡很可能有不少是這樣來的。**落差在於量級不對等**:多行文字貼上有確認對話框,而在別人機器上寫檔案沒有。修法不是加對話框而是**分開兩條路**:滑鼠貼上(右/中鍵)只貼文字,圖片上傳保留給明確的 ⌘V 與選單 Paste——xterm 那個習慣本來就是給文字用的,誤觸的代價因此回到「打錯字」,而刻意使用時也不會被對話框拖慢。兩個手抄的 subclass 都改,即使本機 shell 根本不會上傳——它們已經是複製貼上的關係,讓它們在這裡不一致就是漂移的開始。）
（**貼進終端機的截圖會永遠留在遠端,現在保留 7 天**。使用者問「`~/.macmoba/paste-*.png` 為什麼有這些」——那是 ⌘V 貼圖片時,MacMoba 用同一條 SSH 的 SFTP 上傳到**遠端**家目錄的檔案,再把路徑打進提示字元(給 agent 讀)。所以會出現在**你連進去的那台**上,不是本機。但**從來沒有人清理**:開發機上累積了 30 個檔案、9.6MB、跨兩週,而且從外面看只是一堆 `paste-<unix timestamp>.png`,分不出是誰、哪個 session、貼的是什麼。現在上傳成功後會掃一次,刪掉超過 7 天的。**刻意做得很窄**:只認 `paste-<純數字>.png` 這個我們自己寫出來的形狀,別的檔案再舊也不碰;名字解析不出來就留著,不用猜的;年齡看**檔名裡的時間戳**而不是 mtime(那是貼上的當下、是我們自己選的值,不會被後來的傳輸動到);未來時間的檔案不刪(時鐘偏移不該觸發刪除);邊界是「剛好 7 天留著」。掃描在**上傳成功之後**才做,而且所有錯誤都吞掉——在別人的機器上做家務,不值得為它弄丟一張圖。規則本身 6 個純函式測試,加 1 個對真 SFTP server 的測試證明刪除真的到得了對面、而且真的沒碰別人的檔案。）
（**Mac 複製、RDP Windows 貼不上——一次複製送了兩份 format list**(2.27 修好,使用者實機驗收通過:**文字、圖片、檔案三種都可以**——它們本來就共用同一條宣告路徑,所以一起壞、也一起好)。使用者的 log 直接把病灶指出來,而且是**時間戳**說話的:`offer_files: files=0` 和 `offer_clipboard: text=1` 相隔 **1 毫秒**。兩個函式各自都會送一份 format list,而第一份是用**上一次**的內容算的——所以剛複製完那一刻送出去的是一份**空的 format list**,在 MS-RDPECLIP 裡那句話是「我的剪貼簿現在空了」,緊接著第二份才說有文字。順序本身也違規:一份 format list 要等對方回 response 才能送下一份。Windows 留下了錯的那一份。修法:`macmoba_rdp_offer_all()` 把檔案／文字／圖片一起準備好,**一次變動只送一次宣告**。⚠️ 另外那行 `text=1` 的 log **不是「送成功」的證據**——它印在 `send_format_list` 之前,只證明有嘗試;而 `send_format_list` 在能力交換完成前會**安靜地 return**。已補上 log:「沒宣告」和「宣告了但被忽略」從外面看一模一樣,而分辨這兩者就是診斷「貼上沒反應」的全部工作。這一輪被這類「安靜地不做事」咬了很多次。⚠️ **這個我自己驗不了**——手邊沒有 Windows server,xrdp 容器也複現不出來,只能靠使用者確認。）
（**SFTP 面板「列得出檔案但操作不了」——是我把焦點搶走了,不是 SFTP 壞了**(2.95 修好,使用者實機驗收通過)。前兩個 commit 為了讓本機 shell 一定拿得到鍵盤,把取得焦點搬進 `PaneContainerView.viewDidMoveToWindow`;而那個方法**每次進視窗都會跑**,SwiftUI 又會在任何版面變動時重新掛載容器——**打開檔案瀏覽器就是一次版面變動**。於是面板一出現,pane 立刻把鍵盤搶回去。清單照畫,所以看起來像「SFTP 開得起來但什麼都不能做」:改名、篩選、每一個鍵盤操作要的都是那個被搶走的焦點。修法:**只在第一次進視窗時取得焦點**;`adoptTerminal()` 仍然每次都跑,因為被重新掛載的容器確實每次都要把終端機認回來——這兩件事從來就不是同一個需求,是我把它們混為一談。**找錯方向的教訓**:回報是「sftp 壞了」,我前半段都在確認「SFTP 還是會自己開一條 SSH 連線」——這是真的、看起來也令人安心,卻正好指向錯的地方,因為問題根本不在 SFTP,而在別的東西把鍵盤拿走了。先問「壞掉的樣子是哪一種」才是一步就縮小範圍的做法。）
（**libghostty pane 的貼上:文字與圖片都通了(3.15,使用者驗收)——但這一輪我做得很差,值得記下為什麼**。使用者回報「貼圖片變成貼本機路徑」,我出了**六個版本**才修好,前面幾版全部修在錯的地方:(1) 以為是「有文字就不當圖片」那道 guard 擋掉檔案——**實際在這台檢查剪貼簿才發現 Finder 複製檔案時 `string(forType:.string)` 是 nil**,那道 guard 對複製檔案根本不會觸發;(2) 改成支援「路徑寫成純文字」;(3) 再補上讀不到檔案時退回原始圖片資料。**這三次全都在一個從來沒被呼叫的函式下游。** 真正的病灶是使用者的 log 檔說的:裡面只有 `validate paste: -> enabled`,**沒有任何下游的記錄**——AppKit 問「Paste 能用嗎」、答「能」,然後那個動作**沒有送到這個 view**。改成在 view 上用 `performKeyEquivalent` 直接攔 ⌘V 才通。**接著又壞了一次**:攔下來之後文字改走 `sendText`,但本來會動的是 libghostty 自己的 `paste_from_clipboard`——圖片通了文字反而不通,那個「一邊好一邊壞」正好指出分界線。最後:圖片走 App 的上傳,文字走 libghostty 的貼上。**兩個過程教訓**:(a) 中途發現 `override func paste(_:)` 被我後續某次編輯洗掉、3.09–3.13 全都少了它而我沒發現——**現在編輯完會 grep 確認才建置**;(b) 我先做了兩版把記錄寫進 unified logging 的除錯版,使用者那台 `log show` **一行都看不到**(連 libghostty 自己的都沒有),白費兩輪;改成寫檔案(`~/Documents/MacMoba Logs/MacMoba-Paste.log`)之後**一次就定位**。⚠️ **正確的第一步應該是把剪貼簿內容和呼叫路徑印進檔案**,而不是從程式碼推論哪條分支會贏。⚠️ **已知缺口**:⌘V 在 libghostty pane 上不再經過多行貼上確認(現在是終端機自己讀剪貼簿),選單的 Paste 仍然會確認。)
（**libghostty pane 上 ⌘V 沒作用——因為那個 view 從來不回答 AppKit 的選單驗證**(3.07 修好,使用者驗收:「⌘V works now」)。⌘V 在 ghostty pane 上**完全不經過 MacMoba 的程式碼**:AppKit 把 `paste:` 送進 responder chain,而它會**先驗證選單項目是否啟用**才讓快捷鍵生效。SwiftTerm 的 view 有 `validateUserInterfaceItem` 會回答「Paste 可以用」;libghostty 的 view **沒有任何驗證**,於是 Edit ▸ Paste 一直是停用的,⌘V 什麼都不做。**症狀的形狀本身就是線索**:打字全程正常——按鍵直接進 view,不經過選單驗證——只有快捷鍵是死的。修法是讓原本為右鍵選單加的那個子類別也回答:Paste 看剪貼簿有沒有文字、Copy 看有沒有選取、**Select All 永遠停用**(action 在 view 上存在但背後 surface API 什麼都不做,啟用一個按了沒反應的項目比停用更糟)。⚠️ **這是用讀的找到的,不是跑出來的**:自動化已經壞掉(對照測試連「打字」都失敗,而使用者顯然打得了字,所以壞的是工具不是 app)。診斷靠**比較兩個引擎**——一邊有驗證一邊沒有,而症狀正好符合「只有走選單驗證的路徑壞掉」。⚠️ 過程中我先做了兩版除錯建置都沒用上:第一版的開關寫成「整個 app 只在建立第一個 pane 時讀一次」,所以使用者設定完再開分頁根本不會生效,產出零筆記錄。**問「你是怎麼貼的」得到「⌘V」這一個字,比兩版除錯建置都有用。**）
（**終端機引擎抽出接縫,libghostty 可以畫一般分頁了**(`terminalEngine` 設定,預設仍是 SwiftTerm)。起因是量出 libghostty 的解析引擎快 **2.32 倍**(0.149s 對 0.346s,14MB 中文進本機 shell)——而且那是**引擎**不是 renderer,因為 SwiftTerm 當時走的是它比較快的 CoreGraphics 路徑。做法照 `PaneContent` 那次的兩步:先抽 `TerminalEngineView` protocol、SwiftTerm 實作、**零行為改變**,790 測試全綠才進第二步。**介面比預期小**——整個 App 對終端機 view 的需求只有十來個操作。已經接過去的:feed、輸入/resize/標題/bell/連結/OSC52 回呼、字級、主題、選取、捲動、**⌘F 搜尋**、read-screen、焦點。本機 shell 也改成自己持有 `LocalProcess`(原本綁在 `LocalProcessTerminalView` 裡),dead-shell 的 Return/Esc 跟著變成引擎無關。libghostty 側為此在 vendored 套件加了 `readAllText()`(上游只給 viewport)。**還沒接的只剩剪貼簿選單**——它從 AppKit responder chain 進來、拿到的是具體 view;以及 Metal 開關,那本來就只有 SwiftTerm 有的概念。**過程中三次「測試全綠但 byte 到不了」**:`LocalProcessTerminalView` 自己就是它的 delegate(接管走 = 打字沒反應)、focus 用一次性 async 去猜 SwiftUI 何時掛好 view(SSH 贏了、本機輸了)、以及 `grep -c "error:"` 會數到警告裡那句 "this is an error in the Swift 6 language mode",於是編譯失敗被當成成功、測到的是舊的 app bundle。**兩個引擎最後都實測過**:本機 shell 與 SSH 都打得進字、grid 正確、read-screen 內容一致。) 測試 **789 項**（3 項 keychain 測試在無互動的終端機下會失敗,見文末）
（**Web 分頁選不了檔案上傳——因為它從來沒有 UI delegate**。使用者回報:`~/.macmoba/paste-*.png` 沒辦法瀏覽選取上傳到網頁伺服器。`WebTab` 設了 `navigationDelegate`,卻**從來沒設 `uiDelegate`**。WebKit 凡是自己畫不出來的東西都去問 UI delegate,沒有 delegate 就**什麼都不做,而且不出聲**——頁面的 `<input type="file">` 要不到 open panel,點下去就被吞掉,欄位永遠是空的。同一個缺口還讓另外兩件事一起是死的,所以一併補上(它們不是三個功能,是同一個缺席的物件):`alert`/`confirm`/`prompt` 全是 no-op,頁面卡在 `confirm()` 就再也不會往下走;`target="_blank"` 與 `window.open` 點了沒反應,而這在內網 console 上佔了不少導覽。open panel **預設開在 `~/.macmoba` 並顯示隱藏檔**——貼上的截圖就落在那裡,而點開頭的資料夾在 open panel 裡不按 ⇧⌘. 根本進不去。每一條路徑都會呼叫 completion handler(含沒有視窗那條),因為 WebKit 會把頁面的 file input 一直卡住直到它被呼叫。**使用者實機驗收通過(2026-09-02):file picker works now**。⚠️ 值得記下的是**我自己驗不到**:System Events 的 click 根本送不進 WKWebView(整片內容區掃過去,頁面自己的 click handler 一次都沒觸發;而 tab 標題是即時跟著 `document.title` 的,所以偵測方法沒問題),螢幕錄製權限也沒有——所以這一項從頭到尾只能靠使用者點一下才知道。)
（**從遠端桌面放開擷取後,點擊全變成右鍵——v2.24 自己造成的回歸,而且只有 Mac 遠端看得出來**。使用者回報:「mac screen sharing session 在 tag ungroup break 後,點進去 mouse 都變成 right click」。不是按鍵對應錯,是**遠端的 Control 卡在按下狀態**——在 macOS 上 ctrl-click 就是右鍵,所以症狀只在 Mac 遠端出現,換成 Windows RDP 根本看不到。成因鏈:RoyalVNC 的 `handleFlagsChanged` 是拿事件的 flags 去跟自己**黏著的 `lastModifierFlags`** 做差集來產生按鍵事件;而 v2.24 為了修「離開後 shell 打不了字」在 `releaseGrab` 裡加了 `makeFirstResponder(nil)`——按 ⌃⌥ 放開時,`handleFlagsChanged` 先呼叫 `releaseGrab()` **才**把事件交回 responder chain,此時 framebuffer view 已經不是 first responder,於是**它永遠沒看到那次「修飾鍵歸零」**,`lastModifierFlags` 停在 `[.control, .option]`,Control 的 keyUp 從來沒送出去。修法是次序:先把 `flagsChanged` 交給那個 view,再交還鍵盤。這個補送是冪等的——等事件走到 responder chain 時 flags 已經一致,差集為空。**注意這條推論預測它與解散分割無關**:任何一次 ⌃⌥ 放開都會踩到,待使用者確認。)
...
（**本機終端機終於可以分割了,而擋住它的正是那個 refactor 自己留下的痕跡**。`canSplit` 是 `!isSinglePane`,而 `isSinglePane` 的定義是 `localTerminal != nil || isFileBrowserOnly`——本機分頁被歸類成「沒有 pane tree」。但它其實**有**一棵樹,只是樹裡放的是一個永遠不會連線的 `TerminalTab` 佔位符,然後 ContentView 繞過那棵樹自己畫 `localTerminal`。**那正是當初引入 `PaneContent` 要消滅的那個佔位符**,只是 VNC/RDP/Web 都改完了,本機終端機被漏掉。所以修法不是放寬 `canSplit`,而是把它補完:`PaneContent` 新增 `.localShell`,本機分頁改成一個普通的葉節點,`localTerminal` 跟 `vnc`/`rdp`/`web` 一樣從樹推導出來。順手清掉五個**同一類的語意漂移**——`title`、`termView`、`aggregateState` 都以 `if let localTerminal` 搶先回傳,現在一個 SSH pane 旁邊放一個 shell 會被整個分頁冠上「Local」而且沒有 ▦n 標記;`supportsTransfer` 用「這個分頁含不含 shell」判斷,會害旁邊的 SSH pane 失去傳檔;字體/主題/GPU 算繪只套用到找到的第一個 shell。`PaneLayout` 新增 `.localShell` case(而不是保留一個假的 session id——本機 shell 沒有 vault session,把它編碼成有,是那種一直撐到有人拿去查表才爆的謊),並補上「整個分頁只有 shell 時沒有 session id 可以當還原起點」這個洞。分割選單新增 New Local Terminal。6 個新測試,共 789 項）
（**自簽憑證的網頁分頁修好了,真正的原因是 ATS,而且它「不可被覆蓋」**。2.19 的 log 是決定性的:`exceptionsSet=true`、`offered` 與 `stored` 指紋相同、`action=accept`、`failingURL` 就是剛剛接受的那台主機(不是登入轉址過去的 oauth),然後**還是 -1202**。也就是說釘選、時機、credential 三件事全都是對的。真兇是 **App Transport Security**,而不明顯的關鍵是:**ATS 的拒絕沒辦法從 authentication challenge 覆蓋**——challenge 照樣送到、credential 照樣被接受、頁面照樣失敗,所以從外面看起來就像「按了信任完全沒用」。**我先前判斷「不是 ATS」是錯的**:那幾輪測試全部連的是 `127.0.0.1` 和一個 LAN IP,而 **ATS 對 IP 位址根本不生效**,所以那些測試從頭到尾都在 ATS 管不到的地方跑。改用真正的主機名稱(`sslip.io`)重測,一次就重現:同一個頁面,裸執行檔(沒有 Info.plist 就沒有 ATS)載入成功,放進 app bundle 就失敗、兩次 challenge 之後 -1202,**跟使用者的 log 一模一樣**;加上 `NSAllowsArbitraryLoadsInWebContent` 之後立刻載入成功。用 `InWebContent` 而不是 `NSAllowsArbitraryLoads`:只豁免網頁內容,App 自己的網路請求仍受 ATS 管;而真正在保護這些分頁的是 `web_certs.json` 的每主機憑證釘選,完全不受影響,憑證換掉一樣會擋。）
...
寫下來的原因:KKTerm 的架構文件裡有一句「切換分頁絕不能斷開或重建活著的
session」——而那正是我們 3.0 出貨、3.2 才修掉的 bug,一字不差。先寫下來的
規則,review 的時候才對得到。

1. **切換分頁、重排版面、開關側欄,不得斷開或重建任何活著的 session。**
   終端機的 view 與 surface 由引擎持有,活得比每一次 SwiftUI 掛載都久
   (3.2 的修法);連線本身由 tab 持有,與畫面無關。
2. **引擎功能一律走被持有的參考,不走 weak 鏡像。**
   `TerminalViewState.surface` 是 weak 的,曾經三次變成 nil 假裝成別的病:
   3.14 貼上失敗、3.15 Copy 停用、3.2 讀不到畫面。
3. **主執行緒不做無界工作。** 檔案列舉、網路、加解密都在 NIO event loop
   或背景 Task;卡住 main thread 的 PR 不收。
4. **秘密不進 log、不進 vault 明文、不進控制通道的回應。**
   `op://`/`cmd:` 在連線當下解析,解析結果不落地。
5. **清單是別人可以改動的東西的快照,不得無限期展示。**
   檔案面板重新出現就重新列;載入失敗就清空,不留舊列假裝還活著(3.3)。
6. **會打字進終端機的自動化一律 opt-in。** MCP 的 `send_text` 躲在
   Settings 開關後面,而且閘門在 app 這側的 socket 上——每個 client 都得
   過它,不能靠 client 自律。

## 效能基準(release build,`./measure.sh`)

量測日 2026-09-17,Mac mini(開發機),v3.4 前夕:

| 指標 | 量到 | 預算 |
|---|---|---|
| 冷啟動 → 控制通道可用 | 285 ms(建置後第一次 1304 ms,dyld cache 冷) | ≤ 600 ms(暖) |
| 控制通道 round-trip | 4 ms | ≤ 10 ms |
| 閒置記憶體(RSS) | 88–93 MiB | ≤ 150 MiB |

每次 release 前跑一次;超預算就是要修的 bug,不是新常態。
`MEASURE_SESSION=<名> ./measure.sh` 可另量「open 到 connected」
(含網路與認證,只當趨勢看)。
