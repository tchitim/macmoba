...
有些內部名稱只存在於某台 DNS,而你的 Mac 和跳板機都碰不到它。這種頁面在還沒
用到隧道之前就失敗了——根本沒有位址可以連。

Chrome 有個旗標專門處理這件事:`--host-resolver-rules="MAP name 10.0.0.1"`。
Web 連線內建同樣的東西:URL 下方的 **Host overrides**,一行一條規則。

```
cp-sim.dev.crp.iclnet2.hk = 10.26.132.82
```

為什麼是「對應」而不是直接把位址打進 URL?因為**名稱本身還是得送出去**:TLS 拿
它比對憑證,而一台主機上放多個網站時,是靠 SNI 和 `Host` 標頭來分辨的。這裡只換
掉隧道另一端實際撥出的位址,瀏覽器完全不知情,照樣送出它原本拿到的名稱。

規則也可以連 port 一起指定,給那種不在網址說的 port 上的服務:
...
**Library（⌥⌘L）→ Macros** 新增。一個巨集就是一段文字，加上一個「**送出後按 Return**」的開關——關掉的話文字會停在提示符等你確認，那正是危險指令該有的行為。

前九個巨集自動取得 **⌃⌘1–9**；更多的仍列在 **Macros 選單**裡。

巨集走的是**跟你手打完全相同的送出路徑**，所以它會遵守 MultiExec 廣播——這是刻意的，但也因此危險：一個巨集配上廣播，一次按鍵就在整個機群上執行。預設會先問一次，可在**設定 → General** 的「Ask before a macro runs on every connected session」關掉。

### MultiExec（廣播輸入）

工具列的 **MultiExec** 鈕，或 **⇧⌘B**。打開之後，你在任何一格打的字會同時送到**所有視窗的每一個已連線終端機**。

**打開時它會把終端機分頁聚成格狀版面**，關掉時再散回原本的分頁。理由很直接：**廣播到你看不見的連線**，正是這個功能最該避免的事。

**個別退出**：廣播進行中，每一格右上角會出現一個天線圖示，點它就把那一格移出群組。**你正在打字的那一格永遠收得到自己的鍵盤**——一個會忽略你打字的終端機不是功能，是故障。

有東西被移出群組時，工具列圖示會變成**斜線版本**，提醒你這次廣播不是送到全部。

遠端桌面與網頁分頁不參與廣播——它們沒有可以打字進去的位元組流。
