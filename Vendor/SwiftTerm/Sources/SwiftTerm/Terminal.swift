...
open class Terminal {
    ...
    private struct ReadingBuffer {
        ...
        // Looks ahead without consuming. Only valid when bytesLeft() covers
        // the offset, which every caller checks first.
        func peek (_ offset: Int) -> UInt8
        {
            let i = idx + offset
            if i < putbackBuffer.count {
                return putbackBuffer [i]
            }
            return rest [i - putbackBuffer.count + rest.startIndex]
        }
    ...
    /// True when the previous cell might end in a ZWJ, the one way a scalar
    /// with no combining class of its own still has to take the general path:
    /// the joiner pulls it into the preceding grapheme cluster.
    ///
    /// A bare ZWJ is zero width and never occupies a cell, so only a cell
    /// holding a whole cluster can end in one — and that is precisely what a
    /// non-simple rune is. Comparing the code is enough; rebuilding the
    /// previous Character, which is what the general path does, is not needed.
    private func previousCellCanJoin (_ buffer: Buffer) -> Bool
    {
        let last = buffer.lastBufferStorage
        guard last.cols == cols && last.rows == rows else { return false }
        let lastx = last.x >= cols ? cols-1 : last.x
        guard lastx >= 0 else { return true }
        return !buffer.lines [last.y][lastx].isSimpleRune
    }
    ...
    func handlePrint (_ data: ArraySlice<UInt8>)
    {
		...
		if chWidth > 0 {
                ...
                // Fast path for standalone East Asian scalars, which are most
                // of what CJK output consists of. The general decode below
                // allocates an array per character, builds a Character, and
                // then walks the grapheme-combining machinery — none of which
                // can apply to a scalar with no combining class that cannot
                // join its neighbour. Only the ZWJ case can still pull such a
                // scalar into the previous cluster, so that is checked.
                //
                // Every qualifying scalar is a three-byte sequence, and a value
                // landing in those blocks cannot have come from an overlong or
                // surrogate encoding, so validating the continuation bytes is
                // the whole of the check needed before trusting the decode.
                if n == 3 {
                    let b1 = readingBuffer.peek (0)
                    let b2 = readingBuffer.peek (1)
                    if (b1 & 0xC0) == 0x80 && (b2 & 0xC0) == 0x80 {
                        let value = (UInt32 (code & 0x0F) << 12)
                                  | (UInt32 (b1 & 0x3F) << 6)
                                  |  UInt32 (b2 & 0x3F)
                        if UnicodeUtil.isStandaloneWideCJK (value),
                           !previousCellCanJoin (buffer),
                           let scalar = UnicodeScalar (value) {
                            _ = readingBuffer.getNext ()
                            _ = readingBuffer.getNext ()
                            buffer.insertCharacter (
                                makeCharData (attribute: curAttr, scalar: scalar, size: 2))
                            continue
                        }
                    }
                }
