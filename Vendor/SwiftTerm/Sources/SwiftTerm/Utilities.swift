...
struct UnicodeUtil {
    ...
    /// East Asian blocks whose scalars are all two columns wide, carry no
    /// combining class, and can never join a neighbour into one grapheme
    /// cluster. They cover the bulk of CJK output, which lets `handlePrint`
    /// skip the general grapheme path for them.
    ///
    /// The bounds are not eyeballed: `CJKFastPathVerification` walks every
    /// scalar in these ranges and checks all three properties. The gaps are
    /// real — U+302A-302F are combining tone marks, and U+303F, U+3040,
    /// U+3097 and U+3098 are one column, not two.
    static func isStandaloneWideCJK (_ value: UInt32) -> Bool
    {
        switch value {
        case 0x3000...0x3029,   // CJK symbols and punctuation
             0x3030...0x303E,   // ditto, after the combining tone marks
             0x3041...0x3096,   // hiragana
             0x309B...0x30FF,   // kana, after the combining sound marks
	...
	// C1 control characters (0x7F-0x9F) return -1
        ...
        // East Asian text is the common non-ASCII case and is uniformly two
        // columns wide, so answering here skips the Unicode property lookup
        // and the bisearch below — the expensive half of this function.
        if isStandaloneWideCJK (irune) {
            return 2
        }
//        if irune < 127 {
//            return 1
